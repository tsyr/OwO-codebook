...
Pt operator+(Pt p1, Pt p2) { return Pt{p1.x + p2.x, p1.y + p2.y, p1.z + p2.z}; }
Pt operator-(Pt p1, Pt p2) { return Pt{p1.x - p2.x, p1.y - p2.y, p1.z - p2.z}; }
type operator*(Pt p1, Pt p2) { return p1.x * p2.x + p1.y * p2.y + p1.z * p2.z; }
Pt operator*(Pt p, type t) { return Pt{p.x * t, p.y * t, p.z * t}; }
Pt operator/(Pt p, type t) { return Pt{p.x / t, p.y / t, p.z / t}; }
type norm2(Pt p) { return p.x * p.x + p.y * p.y + p.z * p.z; }
type norm(Pt p) { return sqrt(p.x * p.x + p.y * p.y + p.z * p.z); }
